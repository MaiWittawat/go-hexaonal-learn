...
func TestLogin(t *testing.T) {
	...
	t.Run("error_login_comparePassword", func(t *testing.T) {
		password := "password"
		user := &entities.User{
			ID:       "1",
			Email:    "mai@example.com",
	...
	t.Run("error_login_createToken", func(t *testing.T) {
		password := "password"
		hash, _ := bcrypt.GenerateFromPassword([]byte(password), 4)
		user := &entities.User{
			ID:       "1",
		...
		userRepoMock.On("FindByEmail", ctx, user.Email).Return(expectedUser, nil)
